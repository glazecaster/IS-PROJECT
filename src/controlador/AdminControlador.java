...
public class AdminControlador {
    private vista.AdminVista vista;
    private ArchivoServicio servicio;

    public AdminControlador(vista.AdminVista vista) {
        this.vista = vista;
    ...
    private void configurarTabla() {
        ...
        vista.tablaDatos.getTableHeader().setForeground(UIConstants.BLUE_DARK);
    ...
    private String construirRegistro() {
        ...
        String prot = vista.txtProt.getText().trim();
        String kcal = vista.txtKcal.getText().trim();
        ...
        if (!prot.isEmpty() && !prot.matches("^\\d+(\\.\\d+)?\\s*(-\\s*\\d+(\\.\\d+)?)?\\s*g$") 
            && !prot.equals("0g") && !prot.equals("0 g")) {
            vista.mostrarMensaje("Formato de proteínas inválido. Use: '25g', '25.5g', '20-30g' o '0g'");
            vista.txtProt.requestFocus();
            return null;
        }
        
        
        if (!kcal.isEmpty() && !kcal.matches("^\\d+(\\.\\d+)?\\s*(-\\s*\\d+(\\.\\d+)?)?\\s*kcal$") 
            && !kcal.equals("0 kcal") && !kcal.equals("0kcal")) {
            vista.mostrarMensaje("Formato de calorías inválido. Use: '350kcal', '350.5 kcal', '300-400kcal' o '0 kcal'");
            vista.txtKcal.requestFocus();
            return null;
        }
        ...
        String protFormateado = formatearValor(prot, "g", "0g");
        String kcalFormateado = formatearValor(kcal, "kcal", "0 kcal");
        
        return String.format("%s|%s|%s|%s|%s|%s|%s", 
                tipo, dia, concepto, monto, periodo, protFormateado, kcalFormateado);
    }
    ...
    private String formatearValor(String valor, String unidad, String valorPorDefecto) {
        ...
        if (valor.endsWith(unidad)) {
            return valor;
        }
        
        
        if ((unidad.equals("g") && valor.endsWith("g")) ||
            (unidad.equals("kcal") && valor.endsWith("kcal"))) {
            return valor;
        }
        
        
        if (valor.matches("^\\d+(\\.\\d+)?\\s*-\\s*\\d+(\\.\\d+)?$")) {
            return valor + " " + unidad;
        }
