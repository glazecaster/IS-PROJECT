...
Nuevo módulo: ComeUCVFaceAccess
- Se añadió un programa independiente para simular el acceso físico al comedor mediante escaneo facial.
- Ejecuta `src_old/faceaccess/App.java` para abrir la interfaz de reconocimiento facial.
- El flujo es: escanear foto → reconocer usuario por su imagen registrada → seleccionar bandeja (desayuno/almuerzo por día) → debitar del monedero → registrar asistencia.
- Durante el registro de un comensal en `app/App.java` ahora se solicita una foto que se normaliza y guarda en `data/faces/`.
- El inicio de sesión normal ya no descuenta automáticamente los 6 dólares de acceso.
