...
public class LoginController {
            ...
            try {
                ...
                if (si) {
                    ...
                    if (tipoLogeado.equals("A")) {
                        // ADMINISTRADOR - Abrir panel de administración
                        abrirPanelAdministrador();
                    } else {
                        // COMENSAL - Abrir vista de comensal
                        abrirVistaComensal(v.txtCedula.getText(), Double.parseDouble(sal));
                    }
                } else {
    ...
    private void abrirPanelAdministrador() {
        // Crear y mostrar la ventana principal de administración
        vista.AdminVista vistaCostos = new vista.AdminVista();
        new AdminControlador(vistaCostos);
    ...
    private void abrirVistaComensal(String cedula, double saldoInicial) {
        view.ComeUCVView mainV = new view.ComeUCVView();
        model.RepositorioSemana repo = new model.RepositorioSemana();
        model.Monedero mon = new model.Monedero(cedula, saldoInicial);
